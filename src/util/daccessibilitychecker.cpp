...
void DAccessibilityCheckerPrivate::printSummaryResults()
{
    ...
    qWarning() << summary.arg(totalWidgetsCount).arg(totalWidgetsCount - this->widgetsWarningList.count() - this->widgetIgnoredCount).arg(this->widgetsWarningList.count()).arg(this->widgetIgnoredCount);
}
